...
$(function () {
  ...
  var renderBooks = function(limit = 12, offset = 0) {
    var content = '';

    Object.keys(books).forEach(function(element, i) {

      if (i >= offset && i < limit) {
      content += '<a href="/book-page.html" class="book d-flex flex-column align-items-center position-relative">' +
        '<div class="book__mark position-absolute">';

        if (i >= offset && i < (offset + 4)) {
          content += '<img src="img/new.png" alt="new">';
        }
        content += '</div>' +
        '<div class="book__img">' +
